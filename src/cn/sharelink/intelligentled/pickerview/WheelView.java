...
public class WheelView extends View {
	...
	public WheelView(Context context, AttributeSet attrs) {
		 ...
		 * textColorCenter
		 * =getResources().getColor(R.color.pickerview_wheelview_textcolor_center
		 * ); dividerColor =
		 * getResources().getColor(R.color.pickerview_wheelview_textcolor_out);
		 */

		textSize = getResources().getDimensionPixelSize(
				R.dimen.pickerview_textsize);// 默认大小
	...
	protected void onDraw(Canvas canvas) {
		...
		if (dividerType == DividerType.WRAP) {// 横线长度仅包裹内容
			...
			canvas.drawLine(startX, firstLineY, endX, firstLineY,
					paintIndicator);
			canvas.drawLine(startX, secondLineY, endX, secondLineY,
					paintIndicator);
		} else {
		...
		while (counter < itemsVisible) {
			...
			// 求弧度 α = L / r (弧长/半径) [0,π]
			double radian = ((itemHeight * counter - itemHeightOffset))
					/ radius;
			// 弧度转换成角度(把半圆以Y轴为轴心向右转90度，使其处于第一象限及第四象限
			// angle [-90°,90°]
			float angle = (float) (90D - (radian / Math.PI) * 180D);// item第一项,从90度开始，逐渐递减到
																	// -90度
			...
			if (angle >= 90F || angle <= -90F) {
				...
				if (!isCenterLabel
						...
						&& !TextUtils
								.isEmpty(getContentText(visibles[counter]))) {
				...
				if (translateY <= firstLineY
					...
					canvas.drawText(contentText, drawCenterContentStart, Y,
							paintCenterText);
	...
	private void reMeasureTextSize(String contentText) {
		Rect rect = new Rect();
		paintCenterText.getTextBounds(contentText, 0, contentText.length(),
				rect);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		default:
			if (!eventConsumed) {// 未消费掉事件
				 ...
				 * TODO<关于弧长的计算>
				 * 
				 * 弧长公式： L = α*R 反余弦公式：arccos(cosα) = α 由于之前是有顺时针偏移90度，
				 * 所以实际弧度范围α2的值 ：α2 = π/2-α （α=[0,π] α2 = [-π/2,π/2]） 根据正弦余弦转换公式
				 * cosα = sin(π/2-α) 代入，得： cosα = sin(π/2-α) = sinα2 = (R - y) /
				 * R 所以弧长 L = arccos(cosα)*R = arccos((R - y) / R)*R
				 */
